        ...
        $("#entityForm").onsubmit = async (e) => {
            ...
            if (type === "chapter") {
                ...
                x.hobbies = (fd.get("hobbies") || "")
                    ...
                    .map((v) => v.trim())
                    .filter(Boolean);
                x.aliases = (fd.get("aliases") || "")
                    .split(",")
                    .map((v) => v.trim())
                    .filter(Boolean);
                x.personality = (fd.get("personality") || "").trim();
                x.innerConflict = (fd.get("innerConflict") || "").trim();
                x.motivation = (fd.get("motivation") || "").trim();
                x.obsession = (fd.get("obsession") || "").trim();
                x.past = (fd.get("past") || "").trim();
                x.principles = (fd.get("principles") || "").trim();
                x.factions = fd
                    .getAll("cf_faction")
                    .map((factionId, i) => ({
                        factionId,
...
const CREATOR_WIKI_MULTILINE_KEYS = new Set([
    "personality",
    "innerConflict",
    "motivation",
    "obsession",
...
function creatorAppendixEntityPage(st, src, href, opts) {
    ...
    const infobox = fieldEntries
        .map(([key, value]) => {
            const rendered = CREATOR_WIKI_MULTILINE_KEYS.has(key)
                ? creatorWikiText(value)
                : creatorWikiValue(value, href, st, key);
            return rendered
                ? `<div><dt>${creatorHtmlText(creatorWikiLabel(key, creatorWikiHumanize(key)))}</dt><dd>${rendered}</dd></div>`
                : "";
        })
